...
export function Schedule() {
  ...
  const { data, isPending, error } = useQuery<LaunchDataResponse>({
    queryKey: ['launches'],
    queryFn: () => {
      return fetch(`${import.meta.env.VITE_API_URL}${API_ROUTES.LAUNCHES}`).then(response => response.json());
    },
  });
  ...
  return (
    <Card title="Launch Schedule" className="p-5">
      <CardTitle className="flex justify-between">
        <p>Launch schedule</p>
        <Button variant="outline" size="sm" className="font-normal text-sm hover:cursor-pointer" onClick={() => navigate('/launch-schedule')}>
          View all
        </Button>
      </CardTitle>
      {isPending && (
