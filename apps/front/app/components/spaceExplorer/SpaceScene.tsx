...
const ORBIT_ELEMENTS: Partial<Record<BodyName, { a: number; e: number; i: number; Omega: number; omega: number }>> = {
  Mercury: { a: 0.387098, e: 0.205630, i: 7.00487,  Omega: 48.33167,  omega: 29.12478 },
  Venus:   { a: 0.723332, e: 0.006772, i: 3.39471,  Omega: 76.68069,  omega: 54.85229 },
  Earth:   { a: 1.000000, e: 0.016710, i: 0.00005,  Omega: 348.73936, omega: 114.20783 },
  Mars:    { a: 1.523679, e: 0.093400, i: 1.85061,  Omega: 49.57854,  omega: 286.46230 },
  Jupiter: { a: 5.20260,  e: 0.048498, i: 1.30327,  Omega: 100.55615, omega: 273.86700 },
  Saturn:  { a: 9.55491,  e: 0.055508, i: 2.48524,  Omega: 113.71504, omega: 339.39200 },
  Uranus:  { a: 19.2184,  e: 0.046381, i: 0.773059, Omega: 74.00595,  omega: 96.998857 },
  Neptune: { a: 30.1104,  e: 0.008988, i: 1.76917,  Omega: 131.78400, omega: 273.18700 },
}
...
function EllipticalOrbit({ body, elements, segments = 512, color = '#2a2f45', sunOffset }: { body: BodyName; elements: { a: number; e: number; i: number; Omega: number; omega: number }; segments?: number; color?: string; sunOffset: [number, number, number] }) {
  const points = useMemo(() => {
    ...
    // Rotation from orbital plane (ecliptic) to equatorial J2000:
    ...
    for (let k = 0; k <= segments; k++) {
      ...
      const z_plane = 0
      // Rotate to ecliptic space then to equatorial
      const vEq = mulMatVec(R, [x_plane, y_plane, z_plane])
      // Scale and center on Sun
      pts.push([
        vEq[0] * AU_TO_UNITS - sunOffset[0],
        vEq[1] * AU_TO_UNITS - sunOffset[1],
        vEq[2] * AU_TO_UNITS - sunOffset[2],
      ])
    }
    return pts
  }, [elements, segments, sunOffset])
  return <Line points={points} color={color} lineWidth={1} opacity={0.6} transparent />
}

function bodyColor(body: BodyName): string {
  switch (body) {
    case 'Sun': return '#ffaa00'
    case 'Mercury': return '#b3b3b3'
    case 'Venus': return '#d9c58b'
    case 'Earth': return '#3fa7ff'
    case 'Moon': return '#dddddd'
    case 'Mars': return '#d14b3f'
    case 'Jupiter': return '#c89f6c'
    case 'Saturn': return '#d3c7a6'
    case 'Uranus': return '#7fd1d8'
    case 'Neptune': return '#5a79d6'
...
function Planet({ obj, selected, onSelect }: { obj: ObjectPositionDTO; selected: boolean; onSelect: (o: ObjectPositionDTO, position: [number, number, number])=>void }) {
  ...
  const position = useMemo(() => [
    obj.vectorAu.x * AU_TO_UNITS,
    obj.vectorAu.y * AU_TO_UNITS,
    obj.vectorAu.z * AU_TO_UNITS,
  ] as [number, number, number], [obj.vectorAu])

  // Physically-informed radius with clamped visual scaling
  const radius = useMemo(() => physicalRadiusUnits(obj.body), [obj.body])

  const handleClick = useCallback((e: any) => { e.stopPropagation(); onSelect(obj, position) }, [onSelect, obj, position])
    ...
    <group position={position}>
      ...
      {selected && (
        <Html position={[0, radius * 1.6, 0]} center distanceFactor={6} style={{ pointerEvents: 'none' }}>
          <div style={{ background: 'rgba(10,12,20,0.9)', border: '1px solid rgba(255,255,255,0.15)', padding: '8px 10px', borderRadius: 8, color: '#e6e6e6', minWidth: 160, boxShadow: '0 6px 16px rgba(0,0,0,0.45)' }}>
            ...
            {typeof obj.observables.magnitude === 'number' && (
              <div style={{ fontSize: 12, opacity: 0.9 }}>Mag: {obj.observables.magnitude.toFixed(1)}</div>
            )}
...
export default function SpaceScene({ objects }: SpaceSceneProps) {
  ...
  const onSelect = useCallback((o: ObjectPositionDTO, pos: [number, number, number]) => { setSelected(o.body); setTargetPos(pos) }, [])
  const isSelected = useCallback((o: ObjectPositionDTO) => selected === o.body, [selected])

  // Sun at origin
  const sunOffset: [number, number, number] = [0, 0, 0]

  return (
    <div className="w-full h-[70vh] rounded">
    <Canvas camera={{ position: [0, 15, 30], fov: 60 }} className="w-full h-full bg-[#02030a] rounded" onPointerMissed={() => { setSelected(null); setTargetPos(null) }}>
      ...
      <Suspense fallback={null}>
        ...
        {Object.entries(ORBIT_ELEMENTS).map(([body, el]) => (
          <EllipticalOrbit key={`orbit-${body}`} body={body as BodyName} elements={el!} color={bodyColor(body as BodyName)} sunOffset={sunOffset} />
        ))}
        {/* Bodies */}
        {objects.map((o) => (
