...
export default function LaunchSchedulePage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-gray-600 mb-8">Stay updated with the latest launch schedules and mission details.</p>
      <Separator />
      ...
      {isLoading ? (
        ...
        <div className="text-center p-6 bg-red-50 rounded-lg">
          <p className="text-red-600">{error instanceof Error ? error.message : 'Failed to fetch launch schedule. Please try again later.'}</p>
        </div>
      ) : (
        <div className="overflow-x-auto">{data && <TableSchedule data={data.results} />}</div>
      )}
    </div>
  );
}
