...
export default class ESAScrapingService {
  ...
  private static readonly SCRAPING_CONFIGS: Record<string, ESAScrapingConfig> = {
    JWST: {
      ...
      creditsSelector: '#body > div > div.col-md-12.col-lg-12.col-xl-64.left-column.px-md-4.px-xl-3.tex-justify.order-1.order-sm-1.order-lg-1.order-xl-3 > div.credit > p',
      fovSelector: '#object_astrometry_div > table > tbody > tr:nth-child(3) > td',
      constellationSelector: '[aria-describedby="About the Object"] > tbody > tr:nth-child(1) > td > a',
      releaseDateSelector: '[aria-describedby="About the Image"] > tbody > tr:nth-child(3) > td',
      totalPagesSelector: '#dark-body > div > div > div.row.statusbar.mx-5.mt-5 > div.col-md-6 > div > ul > li:last-child',
      type: 'JWST'
    },
    HUBBLE: {
      domain: 'esahubble',
      baseUrl: 'https://esahubble.org/images/archive/search/',
      parameters: '?minimum_size=0&ranking=0&facility=2&id=&release_id=&published_since_day=&published_since_month=&published_since_year=&published_until_day=&published_until_month=&published_until_year=&title=&subject_name=&description=&credit=&fov=0',
      imageLengthSelector: 'picrow',
      imageSelector: '#dark-body > div > div > div.image-list.image-list-300 > div:nth-child(INDEX) > a:nth-child(1)',
      imageFullSizeSelector: '#body > div.row.page > div.col-md-3.right-column > div:nth-child(3) > div:nth-child(2) > span.archive_dl_text > a',
      imageTitleSelector: '#dark-body > div > div > div.image-list.image-list-300 > div:nth-child(INDEX) > a:nth-child(1) > div',
      imageEsaIdSelector: '[aria-describedby="About the Image"] > tbody > tr:nth-child(1) > td',
      ...
      constellationSelector: '[aria-describedby="About the Object"] > tbody > tr:nth-child(3) > td > a',
      releaseDateSelector: '[aria-describedby="About the Image"] > tbody > tr:nth-child(3) > td',
      totalPagesSelector: '#dark-body > div > div > div.row.statusbar > div.col-md-6 > div > ul > li:nth-child(8) > a',
      type: 'HUBBLE'
    }
  }
  ...
  async performInitialHeavyScraping(): Promise<void> {
    ...
    try {
      ...
      // Ensure constellations are seeded
      await Constellation.seedCommonConstellations()
      
      // Scrape all ESA telescope images
      const allImages: ScrapedImageData[] = []
      
      for (const [telescope, config] of Object.entries(ESAScrapingService.SCRAPING_CONFIGS)) {
   ...
   */
  private async scrapeRecentTelescopeImages(config: ESAScrapingConfig): Promise<ScrapedImageData[]> {
  ...
  private async getImageDetails(
    ...
    const constellation = await this.extractText(page, config.constellationSelector)
  ...
  private createImageUrl(url: string, domain: string): string {
    const baseUrl = `https://cdn.${domain}.org/archives/images/large`
   ...
   */
  private async extractAttribute(page: Page, selector: string, attribute: string): Promise<string | null> {
    try {
  ...
  /**
   * Batch save images to optimized database
   */
  private async batchSaveImages(data: ScrapedImageData[]): Promise<void> {
    if (data.length === 0) return

    logger.info(`Saving ${data.length} images to optimized database...`)
