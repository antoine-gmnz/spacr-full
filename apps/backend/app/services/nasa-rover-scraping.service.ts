...
export default class NasaRoverScrapingService {
  ...
  public async scrapeRange(params: {
    ...
    if (!roverRecord) {
      logger.error(`Rover not found in DB: ${rover}`)
      throw new Error(`Rover not found in DB: ${rover}`)
    }
    console.log(
      'HEEEEEEYYYYYYYYYYYYYYYYYYYYYYYYYYZZZZZZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZSZS'
    )
    ...
    for (let sol = beginSol; sol <= endSol; sol++) {
      try {
        ...
        for (const photo of data.photos) {
          try {
            logger.info('NASA photo: %o', photo)
